#!/usr/bin/env python3
# Assembly of the morphometric data frame and export to CSV.
"""Assemble the morphometric data frame from the profile tables."""
import os
import sys
import numpy as np
import pandas as pd
...
# sign of the cross-profile distance on the landward side, per Table 1
...
def read_profiles(path):
    """Yield (index, dist_km, depth_m) for each segment of a grdtrack table."""
    seg, k = [], -1
...
for fn in sorted(os.listdir(PROFILE_DIR)):
    ...
    for k, dist, depth in read_profiles(os.path.join(PROFILE_DIR, fn)):
        if np.isnan(depth).any():
            rejected["window"] += 1
            continue
        if abs(dist[np.argmin(depth)]) > 1.0:      # axis mismatch, Section 4.1
            rejected["axis"] += 1
            continue
